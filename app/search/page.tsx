...
import { Search, X, Star, Filter, Loader2, ChevronLeft, ChevronRight, Info, Calendar, Clock } from "lucide-react"
...
export default function SearchPage() {
  ...
  const [searchResults, setSearchResults] = useState<any>({ media: [], pageInfo: null })
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-md border-b border-zinc-800">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/login">
              <Button className="bg-purple-600 hover:bg-purple-700 hover:cursor-pointer text-white">Sign In</Button>
            </Link>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <form onSubmit={handleSearch} className="mb-8">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <Button type="submit" className="bg-purple-600 hover:bg-purple-700 hover:cursor-pointer py-6 px-8">
                Search
              </Button>
              <Button
                type="button"
            ...
            {showFilters && (
              <div className="mt-4 p-4 bg-zinc-900/60 border border-zinc-800 rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Sort By */}
                  <div>
                    <label className="block text-sm font-medium mb-2 text-zinc-400">Sort By</label>
                  ...
                  {/* Genre */}
                  <div>
                    <label className="block text-sm font-medium mb-2 text-zinc-400">Genre</label>
                  ...
                  <div className="mt-4 pt-4 border-t border-zinc-800">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {status && (
                        ...
                        >
                          Status: {ANIME_STATUSES.find((s) => s.value === status)?.label || status}
                      ...
                      {sort !== "POPULARITY_DESC" && (
                        <Badge
                          className="bg-purple-600/20 text-purple-400 hover:bg-purple-600/30 hover:cursor-pointer"
                          onClick={() => updateFilters("sort", "POPULARITY_DESC")}
                        >
          ...
          {isLoading && searchResults.media.length === 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-zinc-400 mb-4">We couldn't find any anime matching "{query}"</p>
              <p className="text-zinc-500 text-sm">Try using different keywords or adjusting your filters</p>
            </div>
              ...
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold">
                  ...
                  {searchResults.pageInfo?.total && (
                    <span className="text-zinc-400 text-base ml-2">({searchResults.pageInfo.total} found)</span>
                  )}
                </h2>
                <div className="text-sm text-zinc-400">
                  Page {searchResults.pageInfo?.currentPage || 1}
                  {searchResults.pageInfo?.lastPage && ` of ${searchResults.pageInfo.lastPage}`}
                </div>
              </div>
                      ...
                      <div className="relative aspect-[2/3]">
                        <Image
                          src={
                            ...
                            getPlaceholderImage(300, 450, anime.title?.userPreferred || "Anime")
                          }
                          alt={anime.title?.userPreferred || anime.title?.english || anime.title?.romaji || "Anime"}
                          fill
                ...
                <div className="mt-12 flex justify-center items-center">
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center space-x-1">
                      {Array.from({ length: Math.min(5, searchResults.pageInfo.lastPage || 1) }, (_, i) => {
                        ...
                        if (searchResults.pageInfo.lastPage <= 5) {
                          ...
                          pageNum = i + 1
                        } else if (page >= searchResults.pageInfo.lastPage - 2) {
                      ...
                      {searchResults.pageInfo.lastPage > 5 && page < searchResults.pageInfo.lastPage - 2 && (
                        <>
                          <span className="text-zinc-500">...</span>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handlePageChange(searchResults.pageInfo.lastPage)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-center">
                <div className="bg-zinc-900/60 border border-zinc-800 rounded-lg p-4 hover:border-purple-500/50 transition-colors">
                  ...
                  <p className="text-xs text-zinc-500">
                    Try searching by title, genre, or season. Use filters to narrow down results.
                  </p>
                </div>
                <div className="bg-zinc-900/60 border border-zinc-800 rounded-lg p-4 hover:border-purple-500/50 transition-colors">
                  <h3 className="font-medium mb-1">Popular Genres</h3>
                  <div className="flex flex-wrap justify-center gap-1 mt-2">
                    {["Action", "Romance", "Comedy", "Fantasy", "Slice of Life"].map((genre) => (
                      <Badge
                        key={genre}
