...
export default function AnimePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Fetch anime info
        const infoData = await anilist.fetchAnimeInfo({ id: params.id, isCached: true })
        ...
        // Fetch episodes
        const episodesData = await anilist.fetchAnimeEpisodes({ id: params.id, isCached: true })
        if (episodesData) {
        ...
        const trendingData = await anilist.fetchTrendingAnime({ isCached: true })
  ...
  const title = animeInfo.title.userPreferred || animeInfo.title.english || animeInfo.title.romaji
  const japaneseTitle = animeInfo.title.native
  const coverImage =
    animeInfo.coverImage?.extraLarge || animeInfo.coverImage?.large || "/placeholder.svg?height=600&width=400"
  const bannerImage = animeInfo.bannerImage || "/placeholder.svg?height=400&width=1200"
  const description = animeInfo.description?.replace(/<[^>]*>/g, "") || "No description available."
  const genres = animeInfo.genres || []
  const rating = animeInfo.averageScore ? (animeInfo.averageScore / 10).toFixed(1) : "N/A"
  const popularity = animeInfo.popularity ? `#${animeInfo.popularity}` : "Unknown"

  // Format studio names
  const studios = animeInfo.studios?.nodes?.map((studio: any) => studio.name) || []
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="pt-16">
        ...
        <div className="relative h-[300px] md:h-[400px]">
          <Image src={bannerImage || "/placeholder.svg"} alt={title} fill className="object-cover" priority />
        ...
        <div className="container mx-auto px-4 -mt-32 relative z-10">
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="flex-1 space-y-6">
              <div>
                ...
                {japaneseTitle && <p className="text-zinc-400 mt-1">{japaneseTitle}</p>}
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-zinc-400">Popularity: {popularity}</div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p>{animeInfo.duration ? `${animeInfo.duration} min per ep` : "Unknown"}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <Tabs defaultValue="episodes" className="space-y-6">
              ...
              <TabsContent value="episodes" className="mt-6 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  {animeInfo.episodes && episodes.length > 0 && (
                    <div className="text-sm text-zinc-400">
                      <span className="font-medium text-white">{episodes.length}</span> / {animeInfo.episodes} Episodes
                    </div>
                  )}
                </div>

                {animeInfo.episodes && episodes.length > 0 && (
                  <Progress value={(episodes.length / animeInfo.episodes) * 100} className="h-1 bg-zinc-800" />
                )}
                ...
                {isLoading.episodes ? (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row">
                          <div className="relative w-full sm:w-[180px] h-[100px] sm:h-auto">
                            <Image
                              src={episode.image || "/placeholder.svg?height=100&width=180"}
                                ...
                                <div className="flex items-center gap-4 mt-2 text-sm text-zinc-400">
                                  {episode.createdAt && (
                                    <div className="flex items-center">
                                      ...
                                      {moment(episode.createdAt).format("MMM D, YYYY")}
                                    </div>
                                  )}
                                </div>
                              </div>
                  ...
                  <div className="text-center py-8 text-zinc-400">No episodes available for this anime.</div>
                )}
              </TabsContent>
                ...
                {characters.length > 0 ? (
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {characters.map((character: any) => (
                      <Link href={`/character/${character.id}`} key={character.id} className="group">
                        <div className="bg-zinc-900 rounded-lg overflow-hidden border border-zinc-800 hover:border-purple-500/50 transition-colors text-center p-4">
                          <div className="relative w-24 h-24 mx-auto mb-3 rounded-full overflow-hidden">
                            <Image
                              src={character.image?.medium || "/placeholder.svg?height=150&width=150"}
                          ...
                          <p className="text-xs text-zinc-400">{character.gender || "Character"}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-8 text-zinc-400">No character information available.</div>
                )}
              </TabsContent>
              ...
              <TabsContent value="related" className="mt-6">
                {relatedAnime.length > 0 ? (
                  ...
                  <div className="text-center py-8 text-zinc-400">No related anime available.</div>
                )}
              </TabsContent>
            </Tabs>
          </div>
          ...
          <section className="mt-16 mb-12">
            ...
            <AnimeGrid animeList={trendingAnime} isLoading={isLoading.recommendations} category="Recommended" />
          </section>
        </div>
      </main>
